...
Di_all$words <- sapply(Di_all$V3 %>% as.character() , function(x){tryCatch({cutter[x]}, error=function(err){})})

#讀取stop words檔
fin <- file("../爬完的結果!!/stopwords_tw.txt", open = "r")
stopwords <- readLines(fin , encoding = "UTF8")
stopwords <- c(stopwords,"表示", "報導","提及","攝影")#stopwords 加上表示、報導
