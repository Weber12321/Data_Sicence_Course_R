...
# ---- 資料清理 --------------------------------------------------------------
# 匯入資料組
setwd("/Users/Weber/Documents/GitHub/NTU-CSX-DataScience--Group5/Finalproject/Apple")
data <- read.csv("Ko_Appnews.csv")
...
# 依月份建立子資料組
Yao1<- subset(data , data$month == "1", select = content)
Yao2<- subset(data , data$month == "2", select = content)
Yao3<- subset(data , data$month == "3", select = content)
Yao4<- subset(data , data$month == "4", select = content)
Yao5<- subset(data , data$month == "5", select = content)

# ==== 丁守中
# ---- Jan
docs <- Corpus(VectorSource(Yao1$content))
...
# ---- Feb
docs <- Corpus(VectorSource(Yao2$content))
toSpace <- content_transformer(function(x,pattern){
  return(gsub(pattern," ",x))
})
# 刪去單詞贅字、英文字母、標點符號、數字與空格
...
# ---- Mar
docs <- Corpus(VectorSource(Yao3$content))
toSpace <- content_transformer(function(x,pattern){
  return(gsub(pattern," ",x))
})
# 刪去單詞贅字、英文字母、標點符號、數字與空格
...
# ---- Apr
docs <- Corpus(VectorSource(Yao4$content))
toSpace <- content_transformer(function(x,pattern){
  return(gsub(pattern," ",x))
})
# 刪去單詞贅字、英文字母、標點符號、數字與空格
...
# ---- May
docs <- Corpus(VectorSource(Yao5$content))
toSpace <- content_transformer(function(x,pattern){
  return(gsub(pattern," ",x))
})
# 刪去單詞贅字、英文字母、標點符號、數字與空格
