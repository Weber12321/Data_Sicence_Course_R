...
# ==== 柯文哲
# ---- Jan
docs <- Corpus(VectorSource(Ko1$post))
...
# ---- Feb
docs <- Corpus(VectorSource(Ko2$post))
toSpace <- content_transformer(function(x,pattern){
  return(gsub(pattern," ",x))
})
# 刪去單詞贅字、英文字母、標點符號、數字與空格
...
# ---- Mar
docs <- Corpus(VectorSource(Ko3$post))
toSpace <- content_transformer(function(x,pattern){
  return(gsub(pattern," ",x))
})
# 刪去單詞贅字、英文字母、標點符號、數字與空格
...
# ---- Apr
docs <- Corpus(VectorSource(Ko4$post))
toSpace <- content_transformer(function(x,pattern){
  return(gsub(pattern," ",x))
})
# 刪去單詞贅字、英文字母、標點符號、數字與空格
...
# ---- May
docs <- Corpus(VectorSource(Ko5$post))
toSpace <- content_transformer(function(x,pattern){
  return(gsub(pattern," ",x))
})
# 刪去單詞贅字、英文字母、標點符號、數字與空格
...
# ---- Jun
docs <- Corpus(VectorSource(Ko6$post))
toSpace <- content_transformer(function(x,pattern){
  return(gsub(pattern," ",x))
})
# 刪去單詞贅字、英文字母、標點符號、數字與空格
...
# 畫出文字雲
# 儲存文字雲圖片
png("Ko_Jun.png", width = 300, height = 300 )
...
# 報導量
Ko_textNum <- rbind(nrow(Ko1),nrow(Ko2),nrow(Ko3),nrow(Ko4),nrow(Ko5),nrow(Ko6) )
Ko_textNum %>% as.data.frame()
colnames(Ko_textNum) <- "柯文哲報導量"
rownames(Ko_textNum) <- c("Jan","Feb","Mar","Apr","May","Jun")
Ko_textNum
# 輸出
